...
class _LikedPostsScreenState extends State<LikedPostsScreen> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final posts = await _blogService.fetchLikedPosts();
      if (!mounted) return;
      setState(() {
        _posts = posts;
        _isLoading = false;
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text('Failed to load liked posts', style: TextStyle(color: Colors.grey[600])),
  ...
  Widget _buildCard(BlogPost post) {
    return GestureDetector(
      onTap: () async {
        ...
        _fetch(); // Refresh in case user un-liked the post
      },
      child: Container(
          ...
          children: [
            ...
            if (post.image != null)
              ...
              Container(
                ...
                child: const Icon(Icons.article_outlined, color: Color(0xFFE8A323), size: 36),
              ),
